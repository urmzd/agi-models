#!/usr/bin/env python3
"""
RegisterLM: Linear Genetic Programming meets Language Modeling for Parameter Golf.

Architecture inspired by LGP: narrow register bank (hidden dim) + deep sequential
execution of cheap Fourier-parameterized operations. Expressiveness comes from depth
on narrow state via weight sharing, not wide parallel state.

Key ideas:
- Single shared causal self-attention reused across all recurrent steps
- Fourier basis functions replace discrete register addressing with continuous weighting
- Each "register op" is a tiny bottleneck MLP where up/down projections are constrained
  to the span of Fourier basis functions over the register index space
"""
...
class Hyperparameters:
    ...
    disable_register_ops: bool = bool(int(os.environ.get("DISABLE_REGISTER_OPS", "0")))
...
# Patterns identifying scalar/control tensors (use Adam, not Muon).
CONTROL_TENSOR_NAME_PATTERNS = (
    "attn_scales", "op_scales", "q_gain", "read_coeffs", "write_coeffs",
    "mix", "bias", "out_scale", "fourier_basis",
)
...
class FourierRegisterOp(nn.Module):
    """A single LGP-inspired register operation parameterized by Fourier basis functions.

    Reads from the register bank using Fourier-weighted patterns, mixes through a tiny
    channel bottleneck, and writes back. Total params: n_channels * (4*n_basis + n_channels + 1) + 1.
    """
...
class RegisterLM(nn.Module):
    """Language model using LGP-inspired recurrent register operations.

    Token embedding → (shared attention → Fourier register op) × N steps → output.
    Shared attention is paid once in parameters, reused N times for compute depth.
    Each step has a unique FourierRegisterOp with its own tiny parameter set.
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        qk_gain_init: float,
        activation: str = "gelu",
        disable_register_ops: bool = False,
    ):
        super().__init__()
        ...
        self.disable_register_ops = disable_register_ops
        ...
        # Shared causal self-attention (weights paid once, used N times)
        ...
        if not disable_register_ops:
            # Per-step unique Fourier register ops
        ...
        # Fourier basis (deterministic, frozen after model creation)
        if not disable_register_ops:
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        basis = self.fourier_basis.astype(mx.float32) if not self.disable_register_ops else None

        for step in range(self.num_steps):
            # Shared attention (cross-position communication)
            attn_out = self.shared_attn(self.attn_norm(x))
            x = x + self.attn_scales[step].astype(x.dtype)[None, None, :] * attn_out

            if not self.disable_register_ops:
                # Unique register op (within-position register manipulation)
...
class SplitOptimizers:
    def __init__(self, model: RegisterLM, args: Hyperparameters):
        self.args = args
        params = dict(tree_flatten(model.trainable_parameters()))

        self.embed_key = "tok_emb.weight"

        # Muon: shared attention weight matrices (large 2D tensors)
        self.matrix_keys = [
            k for k, p in params.items()
            if k.startswith("shared_attn.") and k.endswith(".weight") and p.ndim == 2
        ]

        # Adam scalar: everything else (register op params, scales, q_gain)
        self.scalar_keys = [
    ...
    def step(self, model: RegisterLM, grads_tree: dict, step: int, lr_mul: float) -> None:
        params = dict(tree_flatten(model.trainable_parameters()))
...
def main() -> None:
    ...
    model = RegisterLM(
        ...
        disable_register_ops=args.disable_register_ops,
    )
    # Freeze the Fourier basis (deterministic, not learned)
    if not args.disable_register_ops:
    ...
    log(f"architecture:RegisterLM (LGP-inspired)")
    ...
    log(
        f"model_params:{n_params} trainable:{n_trainable} vocab_size:{args.vocab_size} "
        f"dim:{args.model_dim} heads:{args.num_heads} kv_heads:{args.num_kv_heads} "
        f"recurrent_steps:{args.num_recurrent_steps} fourier_basis:{args.n_fourier_basis} "
    ...
    # ==============================================================================
    out_path = out_dir / f"{args.run_id}_registerlm_model.npz"
    ...
    quant_path = out_dir / f"{args.run_id}_registerlm_model.int8.ptz"
